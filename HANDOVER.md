...
### 4.2 i18n — luật cứng
- Mọi chuỗi hiển thị đều qua `next-intl`. **`messages/vi.json` và `messages/en.json` phải khớp key tuyệt đối** (hiện 2725 key mỗi bên).
...
1. **RBAC đã chặn thật toàn app bằng ma trận quyền.** Không còn là "nominal".
   ...
   - **Form thêm nhân sự nay có ô "nhóm quyền" + ô "TCM không trả lương"**: trước đây `createStaff` không gán `roleId` → user mới 0 quyền → rơi thẳng vào vòng lặp trên.
