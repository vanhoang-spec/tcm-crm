...
1. **RBAC đã chặn thật toàn app bằng ma trận quyền.** Không còn là "nominal".
   - **danh mục 96 quyền nằm ở CODE** (`src/lib/permission-catalog.ts`), **grant nằm ở DB** (bảng `role_permission`), sửa ở `/settings/roles` tab **"Ma trận quyền"**. Danh mục để ở code vì mỗi mã phải có một chỗ `requirePermission()` tương ứng — thêm dòng vào DB sẽ tạo quyền không ai kiểm.
   ...
   - **~260 điểm chặn**: 187 server action + 65 page + 8 route API (27/07: gác thêm 4 action ORDER + 6 action task bộ phận + đề xuất thanh toán CTV + trang in báo giá + route xuất báo giá). Guard là `requirePermission("<mã>")` ở **câu lệnh đầu tiên** của mỗi page/action; route API dùng `hasPermission()` rồi trả 403.
   - **Role `ADMIN` là sàn cứng trong code** — luôn đủ 96 quyền, không có dòng grant nào trong DB. Cố ý, để không ai tự khoá mình ra khỏi chính trang sửa ma trận.
   ...
   - Grant mặc định do seed dựng = **đúng quyền mọi người có trước khi bật ma trận** (1205 dòng / 20 role), cố tình không siết sẵn — chính sách thật do BGĐ tick trong ma trận. Hai mã vượt trần (`finance.vendor_payment.over_cap`, `finance.invoice.over_cap`) đã backfill cho **BGĐ + CFO** theo quyết định chủ dự án 27/07/2026.
