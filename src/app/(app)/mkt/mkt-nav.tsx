...
type Labels = { posts: string; plan: string; design: string; insights: string };
...
export function MktNav({ labels }: { labels: Labels }) {
  ...
  const onDesign = pathname.startsWith("/mkt/design");

  const tabs = [
    { key: "plan", href: "/mkt/plan", label: labels.plan, active: onPlan },
    { key: "posts", href: "/mkt", label: labels.posts, active: !onInsights && !onPlan && !onDesign },
    { key: "design", href: "/mkt/design", label: labels.design, active: onDesign },
