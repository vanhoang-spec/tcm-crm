...
import { TimelineTemplateCreateForm } from "./template-create-form";
...
export default async function TimelineTemplatesPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
          ...
          <table className="w-full min-w-[560px] text-sm">
            <thead className="sticky top-0 z-10 border-b border-border bg-surface-2 text-left text-xs font-medium text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3">{t("colViewMode")}</th>
            ...
            <tbody className="divide-y divide-border">
              {templates.map((tp) => (
                <tr key={tp.id} className="hover:bg-surface-2/60">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    {tp.viewMode === "CHECKLIST" ? t("viewModeChecklist") : t("viewModeGantt")}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/settings/timeline-templates/${tp.id}`} className="text-xs font-medium text-brand-600 hover:underline">
                      {t("viewEdit")}
                    </Link>
                  </td>
                </tr>
              ...
              {templates.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-6 text-center text-sm text-muted-foreground">—</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <TimelineTemplateCreateForm projectTypes={(projectTypeSet?.items ?? []).map((i) => ({ id: i.id, label: pickLabel(i, locale) }))} />
    </div>
  );
}
