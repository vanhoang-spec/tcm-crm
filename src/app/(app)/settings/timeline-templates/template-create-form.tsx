...
import { createTimelineTemplate, type TimelineTemplateFormState } from "./actions";

export function TimelineTemplateCreateForm({ projectTypes }: { projectTypes: { id: string; label: string }[] }) {
  const [state, formAction, pending] = useActionState<TimelineTemplateFormState, FormData>(createTimelineTemplate, {});
  ...
  return (
    <form action={formAction} className="space-y-3 rounded-xl border border-dashed border-border-strong bg-surface p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <select name="viewMode" defaultValue="GANTT" className="h-9 w-full rounded-lg border border-border-strong bg-surface px-2.5 text-sm">
