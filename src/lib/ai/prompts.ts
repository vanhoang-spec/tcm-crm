...
// ───────────────────────── Khuôn tài liệu có cấu trúc (dùng chung 6 công cụ) ─────────────────────

/**
 * Bắt AI trả JSON theo khuôn `docSchema` (lib/doc-blocks.ts) thay vì văn xuôi.
 *
 * Vì sao gộp một chỗ rồi CHÈN vào prompt sẵn có, thay vì sửa 6 hàm prompt: khuôn khối là thứ chung
 * cho mọi công cụ và sẽ còn đổi (thêm loại khối); rải vào 6 chỗ là 6 chỗ phải nhớ sửa cùng lúc.
 *
 * ⚠ Nhắc "KHÔNG dùng ký tự markdown" là bắt buộc: model quen trả `**đậm**` và `- gạch đầu dòng`
 * ngay trong text của block, mà repo KHÔNG có bộ render markdown (chú thích ở `ai-shared.tsx`) nên
 * người dùng sẽ thấy nguyên mấy dấu sao trong file Word xuất ra.
 */
const DOC_JSON_FORMAT = `
...
/** Chèn khuôn JSON vào cuối message system của một prompt sẵn có. */
...
${DOC_JSON_FORMAT}` };
  ...
  // prompt nào không có system message thì thêm hẳn một cái ở đầu
