...
/**
 * DỰNG FILE WORD (.docx) TỪ BLOCKS — cho mọi thứ AI sinh ra ở `/ai`.
 *
 * Vì sao KHÔNG dùng `docxtemplater` (đã có sẵn trong repo, đang chạy ở `lib/ctv.ts`): thư viện đó
 * ĐIỀN GIÁ TRỊ vào một mẫu .docx cố định — hợp với biểu mẫu BM06/BM09 có sẵn ô trống. Tài liệu AI
 * sinh ra thì SỐ KHỐI và LOẠI KHỐI thay đổi từng lần (lúc có bảng, lúc chỉ gạch đầu dòng), không có
 * mẫu cố định nào khớp. Nên dựng thẳng OOXML rồi nén bằng `pizzip` (cùng dependency, không thêm gì).
 *
 * ⚠ File này Node-only (pizzip) — KHÔNG import vào component `"use client"`, đúng như `lib/ctv.ts`.
 *
 * Đã kiểm bằng cách ĐỌC NGƯỢC: file dựng ra được `mammoth` (thư viện đọc .docx thật, đang dùng ở
 * `ai/extract-text.ts`) bóc lại đúng nội dung — nếu OOXML sai khuôn thì mammoth ném lỗi hoặc trả
 * thiếu chữ, nên đây là phép thử thật chứ không phải chỉ kiểm file có nén được không.
 */
...
/**
 * Ky tu dieu khien (tru tab/xuong dong): Word TU CHOI MO file co chung, va DeepSeek thi thoang tra
 * ve chung lan trong van ban. Dung new RegExp(chuoi) chu khong viet regex truc tiep de ma nguon
 * file nay chi con ASCII — go escape truc tiep de lot ky tu dieu khien THAT vao source (da vap).
 */
const CONTROL_CHARS = new RegExp("[\u0000-\u0008\u000B\u000C\u000E-\u001F]", "g");

/** XML escape. Bỏ ký tự điều khiển: Word từ chối mở file có chúng, và AI thi thoảng trả . */
...
export type DocxFooter = {
  /** Dòng nhỏ cuối tài liệu — dùng để đóng dấu "do AI soạn, người đọc phải kiểm lại". */
  note?: string | null;
};
